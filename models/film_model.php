 ...
 */
function countAllFilm($searchMediaName = null)
{
    try
    {
        $db = DB::getConnect();
        $reg = 'SELECT COUNT(media_Id) as total FROM media WHERE valid = 1 AND media_Titre LIKE :searchname AND typeMedia_Id = 1';
        $st = $db->prepare($reg);
 ...
 * @return array
 */
function searchFilm($mediaName)
{
    try
    {
        $db = DB::getConnect();
        $reg = 'SELECT DISTINCT media_Titre as title FROM media WHERE media_Titre LIKE :searchname AND typeMedia_Id = 1 LIMIT 10;';
        $st = $db->prepare($reg);
        $st->bindValue(':searchname', '%'.$mediaName.'%', PDO::PARAM_STR);
        $st->execute();
        $array = array(); 
