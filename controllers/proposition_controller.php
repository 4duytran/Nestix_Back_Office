...
if(isset($_GET['term'])){
    $search = $_GET['term'];
    $reponse = searchMediaProposition($search);
...
if(isset($_POST['search']) && !empty($_POST['search']))
{
    if(isset($_POST['csrf']) && hash_equals($_SESSION['token'], $_POST['csrf']))
    {
        ...
        $totalMedia = (int)countAllMediaProposition($search); // count total row of sql request
        $listAllMedias = showAllMediaProposition($start, $limit, $search);
...
{
    $totalMedia = (int)countAllMediaProposition(); // count total row of sql request
    $listAllMedias = showAllMediaProposition($start, $limit);
}
